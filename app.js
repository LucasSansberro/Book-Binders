...
//Cargamos el listado al DOM
  for (const opcion of opcionesDeReparacion) {
    function cargarListaDom (ul){
    ul.innerHTML += `<li class="list-group-item clarisimo d-flex justify-content-between"> ${opcion.nombre + opcion.precio}
                    <button id="${opcion.id}"class="botonAgregarEliminar crema"> <i class="fa-solid fa-plus"></i> </button></li>`
    }
    if (opcion.categoria == "Rústica") { cargarListaDom(ulRustica) }
    else if (opcion.categoria == "Cartoné") { cargarListaDom(ulCartone) }
    else if (opcion.categoria == "Cuero") { cargarListaDom(ulCuero) }
    else if (opcion.categoria == "Tamaño") { cargarListaDom(ulTamano) }
    else if (opcion.categoria == "Costura") { cargarListaDom(ulCosturas) }
    else if (opcion.categoria == "Papel") { cargarListaDom(ulPapel) }
  }
...
function render () {
  ...
  carrito.forEach((item, i) => {
    items += `<li class="d-flex justify-content-between"><p class="textoListaCarrito">${item.categoria} | ${item.nombre} ${item.precio}</p>
              <button class="botonAgregarEliminar crema" onclick="botonEliminador(${item.id},${item.precio})"> <i class="fa-solid fa-trash"></i> </button></li>`;
  })
